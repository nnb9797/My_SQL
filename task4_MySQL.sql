...
-- 1. Считаем общее количество лайков, которые получили пользователи младше 12 лет.
SELECT COUNT(likes.id) AS 'likes amount to children less than 12 years old' 
FROM profiles  
JOIN media ON media.user_id = profiles.user_id 
JOIN likes ON media.id = likes.media_id
WHERE 
TIMESTAMPDIFF(YEAR, DATE(profiles.birthday), DATE(current_date)) < 12;   
    
-- 2. Определяем кто больше поставил лайков (всего): мужчины или женщины.
SELECT
CASE (gender)
         WHEN 'm' THEN 'men'
         WHEN 'f' THEN 'women'
    END AS 'Who made more likes', COUNT(*) AS 'likes amount'
FROM profiles
JOIN likes ON profiles.user_id = likes.user_id
GROUP BY profiles.gender
LIMIT 1;

-- 3. Выводим всех пользователей, которые не отправляли сообщения.
-- 1й способ (с помощью NOT IN)
SELECT id,
CONCAT(firstname, ' ', lastname) AS 'people, who send no messages' 
FROM users 
WHERE id NOT IN (
SELECT from_user_id 
	FROM messages);

-- 2й способ (c помощью NOT EXISTS)
SELECT  id,
CONCAT(firstname, ' ', lastname) AS 'people, who send no messages' 
FROM users 
WHERE NOT EXISTS 
(SELECT*FROM messages WHERE users.id = messages.from_user_id);

-- 4.Задаем  пользователя, (пусть это будет пользователь с id = 1, которого зовут Reuben Nienow). 
-- Из всех пользователей соц. сети находим человека, 
-- который больше всех общался с выбранным пользователем (написал ему сообщений).
SELECT id,
CONCAT(firstname, ' ' , lastname) AS 'communicated most of all with Reuben Nienow' 
FROM users WHERE id = (
SELECT from_user_id 
	FROM messages
	WHERE to_user_id = 1
