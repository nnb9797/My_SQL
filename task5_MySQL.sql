...
-- 1. Создаём представление, в которое попадет информация о пользователях (имя, фамилия, город и пол), которые не старше 20 лет.

CREATE OR REPLACE VIEW v_users_table1 AS
SELECT 
	users.firstname AS 'users_firstname',
	users.lastname AS 'users_lastname',
	profiles.hometown AS 'users_hometown',
	profiles.gender AS 'users_gender'
FROM profiles 
JOIN users  
ON users.id = profiles.user_id
WHERE (TIMESTAMPDIFF(YEAR, DATE(profiles.birthday), DATE(CURRENT_DATE))) < 20;

SELECT * FROM v_users_table1;

-- 2. Находим кол-во, отправленных сообщений каждым пользователем и выводим ранжированный список пользователь, 
...
-- Используем DENSE_RANK

CREATE OR REPLACE VIEW v_users_table2 AS
SELECT 
...
SELECT 
	users,
	count(body) AS messages_amount_from_user,
	DENSE_RANK() OVER(ORDER BY count(body) DESC) AS 'user_rank'
FROM v_users_table2
GROUP BY user_number
ORDER BY messages_amount_from_user DESC;

-- 3.Выбираем все сообщения, отсортируем сообщения по возрастанию даты отправления (created_at)
-- находим разницу дат отправления между соседними сообщениями, получившегося списка. 
-- используя LEAD или LAG

CREATE OR REPLACE VIEW v_created_at_table3 AS
SELECT 
	created_at,
	LAG(created_at, 1, 0) OVER (ORDER BY created_at) AS previous_message,
	LEAD(created_at) OVER (ORDER BY  created_at) AS next_message
FROM messages
ORDER BY created_at;

SELECT created_at, previous_message, next_message, 
TIMESTAMPDIFF (MINUTE, created_at, next_message) AS 'difference'
FROM v_created_at_table3;