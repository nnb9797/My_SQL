...
-- 1. Создаём таблицу users_old, аналогичную таблице users. 

DROP TABLE IF EXISTS users_old;
CREATE TABLE users_old (
	id_old SERIAL PRIMARY KEY, -- SERIAL = BIGINT UNSIGNED NOT NULL AUTO_INCREMENT UNIQUE
    firstname_old VARCHAR(50),
    lastname_old VARCHAR(50) COMMENT 'Фамилия',
    email_old VARCHAR(120) UNIQUE
);
-- Создаём процедуру, с помощью которой можно переместить любого (одного) пользователя из таблицы users в таблицу users_old. 
-- (использование транзакции с выбором commit или rollback – обязательно).
START TRANSACTION;
INSERT INTO users_old SELECT * FROM lesson_4.users WHERE id = 2;
COMMIT;
-- проверяем, произошёл ли перенос 
SELECT * FROM users_old;

-- 2. Создаём хранимую функцию hello(), которая будет возвращать приветствие, в зависимости от текущего времени суток. 
